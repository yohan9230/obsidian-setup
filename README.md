...
> 폴더 이름이 `obsidian` 인 이유: 점(`.`)으로 시작하면 대부분의 도구가 숨김 처리해서 다루기 번거롭고, `.gitignore` 가 삼켜 저장소에 올라가지도 않으며, 이 저장소 자체가 볼트로 오인되기 때문이다.
> **이 저장소를 고칠 때 배포물 폴더를 `.obsidian` 으로 개명하면 안 된다** — 위 세 가지가 한꺼번에 터진다.
